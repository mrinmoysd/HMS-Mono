...
const EMPTY = { categoryId: '', itemId: '', supplierId: '', storeId: '', qty: '', purchasePrice: '', date: new Date().toISOString().slice(0, 10), description: '', documentUrl: '' };
...
export function StockView({ onShowItems, onShowIssues }: { onShowItems: () => void; onShowIssues: () => void }) {
  ...
  const cols: Column<ItemStockDto>[] = [
    ...
    { key: 'purchasePrice', header: 'Purchase Price ($)', className: 'tabular text-right', render: (s) => s.purchasePrice.toFixed(2) },
  ];
