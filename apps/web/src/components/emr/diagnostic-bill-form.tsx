...
export function DiagnosticBillForm({ open, modality, title, onClose }: { open: boolean; modality: Modality; title: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/40 p-4 sm:p-8">
      <div role="dialog" aria-modal="true" aria-label={`Generate ${title} Bill`} className="relative w-full max-w-6xl rounded-md bg-surface shadow-xl">
        <div className="flex items-center justify-between border-b border-border px-5 py-3">
          <h2 className="text-base font-semibold">Generate {title} Bill</h2>
